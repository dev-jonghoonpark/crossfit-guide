 ...
 *
 * safety 는 선택 항목이다. { title, points: [] } 를 적으면 동작 페이지 맨 위,
 * 요약 표 바로 아래에 초보자용 주의 상자로 나온다. 다칠 수 있는 동작에만 붙인다.
 */
...
export const movements = [
  ...
  /* ==================================================================== */
  {
    ...
    phases: [
      {
        name: '셋업',
        pose: 'hsSetup',
        desc: '벽에서 한 뼘 정도 떨어진 곳에 손을 어깨너비보다 살짝 넓게 짚고 엉덩이를 접어 올린다. 발은 벽 반대쪽에 두고 차 올릴 준비를 한다.',
        cues: ['손가락을 벌려 바닥을 움켜쥐기', '어깨를 손 위에 놓기', '시선은 손 사이 바닥'],
    ...
    safety: {
      title: '처음이라면 — 목부터 지키세요.',
      points: [
        '체중은 두 손이 받습니다. 정수리는 바닥에 닿기만 하는 정도이고, 목으로 밀어 올리려 하면 안 됩니다.',
        '머리는 두 손보다 앞에 두어 머리·왼손·오른손이 정삼각형을 이루게 합니다. 머리가 두 손과 일직선에 놓이면 체중이 그대로 목에 실립니다.',
        '내려가는 동안 고개를 좌우로 돌리지 말고 시선은 손 사이 바닥 한 곳에 고정합니다. 목이 돌아간 채 체중을 받는 순간이 가장 위험합니다.',
        '앱매트를 1~2장 깔면 가동 범위와 함께 목에 실리는 부담도 줄어듭니다. 처음에는 넉넉히 깔고 시작해서 한 장씩 빼는 편이 안전합니다.',
        '목이나 어깨가 찌릿하거나 저리면 그 자리에서 멈추고 박스 파이크 푸시업·덤벨 프레스로 바꿉니다. 지쳐서 팔이 버티지 못하는 상태로 렙을 이어가다 목으로 내려앉는 사고가 가장 흔합니다.',
      ],
    },
    muscles: [
      { key: 'delts', level: 'primary' },
  ...
  /* ==================================================================== */
  {
    ...
    seoDesc:
      '월 워크는 엎드린 자세에서 손을 한 번에 하나씩 짚어 벽 앞까지 걸어 들어가는 동작입니다. 7단계 분해와 흔한 실수, 입문자용 대체 동작과 하프 월 워크 스케일링을 정리했습니다.',
    thumb: 5,
    ko: '월 워크',
    en: 'Wall Walk',
    abbr: 'WW',
    ...
    intro:
      ...
      '아직 버겁다면 손은 처음 자리에 그대로 둔 채 발만 벽을 타고 올라갔다 내려오는 것으로 대체해도 된다 — 팔로 걸어 들어가지 않으니 어깨에 실리는 체중이 훨씬 적고, 뒤집히는 감각과 코어 버티기는 그대로 배울 수 있다.',
    phases: [
      {
        name: '시작 (엎드림)',
        pose: 'wwStart',
      ...
      },
      {
        name: '밀어 올리기',
        pose: 'wwPlank',
        desc: '팔을 펴 몸을 띄우고 발을 벽에 올린 플랭크를 만든다. 두 손은 아직 나란히 있다.',
      ...
      },
      {
        ...
        pose: 'wwStepA',
        desc:
          '한 손을 벽 쪽으로 한 뼘 옮겨 짚는다. 나머지 한 손은 그 자리에 붙어 있고, 같은 쪽 발이 벽을 한 칸 올라간다. ' +
          '두 손을 같이 움직이면 버틸 곳이 없어져 몸이 무너진다.',
        cues: ['한 번에 한 손만', '옮기는 동안 반대 손은 그 자리', '손 한 뼘 = 발 한 칸'],
        emphasis: ['delts', 'core', 'triceps'],
      },
      {
        name: '반대 손 짚기',
        pose: 'wwStepB',
        desc: '이번엔 반대 손을 옮겨 두 손을 다시 나란히 맞춘다. 손 → 발 → 반대 손 → 반대 발이 한 걸음이다.',
        cues: ['두 손이 다시 나란해지게', '어깨가 손 위에 오도록 따라가기', '골반이 좌우로 돌지 않게'],
        emphasis: ['delts', 'core', 'traps'],
      },
      {
        name: '절반 (하프 월 워크)',
        pose: 'wwHalf',
        desc:
          '몸이 45~60° 정도 서는 지점. 화이트보드의 "Half WW" 스케일은 여기까지만 갔다가 되돌아오는 것이다. ' +
          '여기서부터는 어깨에 실리는 체중이 급격히 늘어난다.',
        cues: ['여기서 숨을 한 번 고르기', '팔꿈치는 계속 편 채로', '갈비뼈 닫고 배 조이기'],
        emphasis: ['delts', 'triceps', 'core'],
      },
      {
        name: '탑',
        pose: 'wwTop',
        desc:
          '두 손이 벽 앞에 모이고 가슴과 허벅지가 벽에 닿는다. ' +
          '손이 코치가 표시해 둔 선(보통 벽에서 25~30cm) 안에 들어오면 그 렙의 탑이 인정된다.',
        cues: ['가슴·허벅지를 벽에 붙이기', '손이 표시선 안에 들어왔는지 확인', '고개는 자연스럽게 떨구기'],
      ...
      },
      {
        ...
        pose: 'wwDown',
        desc:
          '올라올 때와 똑같이 한 손씩 뒤로 짚어 나가고 발은 벽을 타고 내려온다. 무너지듯 떨어지면 어깨가 다친다. ' +
          '가슴이 다시 바닥에 닿아야 1렙이 끝난다.',
        cues: ['내려올 때도 한 번에 한 손', '떨어뜨리지 말고 버티며 내려오기', '가슴이 바닥에 닿아야 1렙'],
    ...
    faults: [
      ...
      { problem: '두 손을 한꺼번에 옮긴다', fix: '순간적으로 버틸 곳이 없어져 몸이 꺾인다. 한 번에 한 손만 옮기고 반대 손은 바닥에 붙여 둔다.' },
    ...
    scaling: [
      '손은 그대로 두고 발만 벽을 타고 올라갔다 내려오기 (팔로 걸어 들어가지 않는다 — 입문자용)',
      '하프 월 워크 (몸이 45~60°가 될 때까지만 걸어 들어갔다 내려오기)',
