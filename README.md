...
| `bike` | 바이크 에르그 안장 — 프레임·플라이휠은 자동 |
| `pedalF` / `pedalB` | 페달 — 크랭크 팔은 자동으로 이어 그린다 |
...
- 월볼(`wb*`) · 바 뮤스클업(`bmu*`) · 월 워크(`ww*`) 계열만 사람을 작게 그린다.
  화면(260×330)에 사람을 같은 크기로 그리면 월볼 타깃(바닥에서 사람 키의 1.6배)이나
  서포트 락아웃이 화면 밖으로 나가 버리고, 월 워크는 탑에서 팔+상체+다리가 통째로
  세로로 쌓여 발이 화면 위로 나간다. 그래서 그 계열만 0.6~0.85배로 줄이고
  `pose(over, dx)` 의 두 번째 인자로 먼 쪽 팔·다리 간격도 같이 줄였다.
- 손에 들린 도구(바벨·덤벨·볼)와 발에 붙은 도구(페달)는 보간할 때 그 관절을 따라간다.
  그래서 페달이 크랭크 원을 도는 동안에도 발에서 떨어지지 않는다.
...
RNL CrossFit 화이트보드 13장에서 뽑았다.

| 분류 | 동작 |
|---|---|
| 역도 | 스쿼트 클린 · 행 스쿼트 클린 · 행 파워 클린 · 스플릿 저크 · 푸시 저크 · 숄더 프레스 · 푸시 프레스 · 클린 앤 저크 · 데드리프트 · SDHP · 벤치 프레스 · 펜들레이 로우 · 파워 스내치 · 행 파워 스내치 · 스쿼트 스내치 · 오버헤드 스쿼트 |
| 체조 | 풀업 · 체스트 투 바 · 바 뮤스클업 · 토투바 · 링 딥 · HSPU · 푸시업 · 디피싯 푸시업 · 에어 스쿼트 · 피스톨 스쿼트 · 월 워크 |
| 덤벨 | 덤벨 스내치 · 덤벨 데드리프트 · 덤벨 파워 클린 · 덤벨 행 파워 클린 · 덤벨 프론트 스쿼트 · 덤벨 파머스 캐리 · 덤벨 S2OH · 덤벨 푸시 프레스 |
| 기타 기구 | 월볼 · 박스 점프 |
| 모노스트럭처 | 더블 언더 · 로잉 · 바이크 에르그 |

와드 13장은 `wods.html` 에서 형식·태그별로 필터링해 볼 수 있다.
